...
public class LA_Ellipsoid : LA_Object, SaveAssets
{
    string _name = "";
    ...
    public LA_Ellipsoid(Vector3 centerPoint, Vector3 dimensions, Vector3 orientation, Color color)
    {
        _parent = new GameObject().transform;
        _parent.gameObject.AddComponent<LA_Object_Component>().laobject = this;


        _ellipsoid = GameObject.CreatePrimitive(PrimitiveType.Sphere).transform;
        _centerPoint = centerPoint;
        _orientation = orientation;
        _rotationFlag = true;
        _ellipsoid.SetParent(_parent);
        ColorScripts.getRightColorScript(color, _ellipsoid.gameObject);
        ...
        _ellipsoid.gameObject.layer = LayerMask.NameToLayer("LA_Object");

        _toStringState = ToStringStates.oneMatrix;
    }

    public LA_Ellipsoid(Transform parent, Transform ellipsoid, Vector3 orientation, Vector3 centerPoint, float scale, Vector3 dimensions)
    {
        _parent = parent;
    ...
    public void draw()
    {
        _ellipsoid.position = _centerPoint;
        if (_rotationFlag) { _ellipsoid.rotation = _ellipsoid.rotation * Quaternion.Euler(_orientation); }
        _rotationFlag = false;
        _ellipsoid.localScale = _dimensions * _scale;
        if (_ellipsoid.lossyScale != Vector3.zero)
        {
            scalingIsZero = false;
        }
        else
    ...
    {
        return Matrix4x4.Rotate(_ellipsoid.transform.rotation);
    }
    ...
    {
        return _ellipsoid.transform.position;
    }

    public Matrix4x4 getMatrix()
    {
        return Matrix4x4.TRS(getTranslation(), _ellipsoid.transform.rotation, Vector3.one * _scale);
    }
    ...
    {
#if UNITY_EDITOR
        string path = "Assets/Resources/" + saveFolder + "/LA_Ellipsoid.prefab";
    ...
    public GameObject getGameObject() { return _ellipsoid.gameObject; }

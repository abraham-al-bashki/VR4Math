...
public class LA_Cuboid : LA_Object, SaveAssets
{
    string _name = "";
    ...
    public LA_Cuboid(Vector3 centerPoint, Vector3 dimensions, Vector3 orientation, Color color)
    {
        _parent = new GameObject().transform;
        _parent.gameObject.AddComponent<LA_Object_Component>().laobject = this;


        _cuboid = GameObject.CreatePrimitive(PrimitiveType.Cube).transform;
        _centerPoint = centerPoint;
        _orientation = orientation;
        _rotationFlag = true;
        _cuboid.SetParent(_parent);
        ColorScripts.getRightColorScript(color, _cuboid.gameObject);
        ...
        _cuboid.gameObject.layer = LayerMask.NameToLayer("LA_Object");

        _toStringState = ToStringStates.oneMatrix;
    }

    public LA_Cuboid(Transform parent, Transform cuboid, Vector3 orientation, Vector3 centerPoint, float scale, Vector3 dimensions)
    {
        _parent = parent;
        _cuboid = cuboid;
        _orientation = orientation;
    ...
    public void draw()
    {
        _cuboid.position = _centerPoint;
        if (_rotationFlag) { _cuboid.rotation = _cuboid.rotation * Quaternion.Euler(_orientation); }
        _rotationFlag = false;
        _cuboid.localScale = _dimensions * _scale;
        if (_cuboid.lossyScale != Vector3.zero)
        {
            scalingIsZero = false;
        }
        else
    ...
    {
        return Matrix4x4.Rotate(_cuboid.transform.rotation);
    }
    ...
    {
        return _cuboid.transform.position;
    }

    public Matrix4x4 getMatrix()
    {
        return Matrix4x4.TRS(getTranslation(), _cuboid.transform.rotation, Vector3.one * _scale);
    }
    ...
    public GameObject getGameObject() { return _cuboid.gameObject; }

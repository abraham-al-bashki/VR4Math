...
public class LA_Mountain : LA_Object, SaveAssets
{
    string _name = "";
    ...
    public LA_Mountain(Vector3 centerPoint, Vector3 dimensions, Vector3 orientation, Color color)
    {
        ...
        GameObject temp = (GameObject)AssetDatabase.LoadAssetAtPath("Assets/Resources/Geometry/Prefab/Mountain.prefab", typeof(GameObject));
        _mountain = ((GameObject)PrefabUtility.InstantiatePrefab(temp)).transform;
#else
        _mountain = GameObject.Instantiate<GameObject>(Resources.Load<GameObject>("Geometry/Prefab/Mountain")).transform;
#endif
        _centerPoint = centerPoint;
        _orientation = orientation;
        _rotationFlag = true;
        _mountain.SetParent(_parent);
        ColorScripts.getRightColorScript(color, _mountain.gameObject);
        ...
        _mountain.gameObject.layer = LayerMask.NameToLayer("LA_Object");

        _toStringState = ToStringStates.oneMatrix;
    }

    public LA_Mountain(Transform parent, Transform mountain, Vector3 orientation, Vector3 centerPoint, float scale, Vector3 dimensions)
    {
        _parent = parent;
        _mountain = mountain;
        _orientation = orientation;
    ...
    public void draw()
    {
        _mountain.position = _centerPoint;
        if (_rotationFlag) { _mountain.rotation = _mountain.rotation * Quaternion.Euler(_orientation); }
        _rotationFlag = false;
        _mountain.localScale = _dimensions * _scale;
        if (_mountain.lossyScale != Vector3.zero)
        {
            scalingIsZero = false;
        }
        else
    ...
    {
        return Matrix4x4.Rotate(_mountain.transform.rotation);
    }
    ...
    {
        return _mountain.transform.position;
    }

    public Matrix4x4 getMatrix()
    {
        return Matrix4x4.TRS(getTranslation(), _mountain.transform.rotation, Vector3.one * _scale);
    }
    ...
    {
#if UNITY_EDITOR
        string path = "Assets/Resources/" + saveFolder + "/LA_Mountain.prefab";
    ...
    public GameObject getGameObject() { return _mountain.gameObject; }

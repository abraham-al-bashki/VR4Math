...
//LA_House
public class LA_House : LA_Object, SaveAssets
{
    string _name = "";
    ...
    public LA_House(Vector3 centerPoint, Vector3 dimensions, Vector3 orientation, Color color)
    {
        ...
        GameObject temp = (GameObject)AssetDatabase.LoadAssetAtPath("Assets/Resources/Geometry/Prefab/House.prefab", typeof(GameObject));
        _house = ((GameObject)PrefabUtility.InstantiatePrefab(temp)).transform;
#else
        _house = GameObject.Instantiate<GameObject>(Resources.Load<GameObject>("Geometry/Prefab/House")).transform;
#endif
        _centerPoint = centerPoint;
        _orientation = orientation;
        _rotationFlag = true;
        _house.SetParent(_parent);
        ColorScripts.getRightColorScript(color, _house.gameObject);
        ...
        _house.gameObject.layer = LayerMask.NameToLayer("LA_Object");

        _toStringState = ToStringStates.oneMatrix;
    }

    public LA_House(Transform parent, Transform house, Vector3 orientation, Vector3 centerPoint, float scale, Vector3 dimensions)
    {
        _parent = parent;
        _house = house;
        _orientation = orientation;
    ...
    public void draw()
    {
        _house.position = _centerPoint;
        if (_rotationFlag) { _house.rotation = _house.rotation * Quaternion.Euler(_orientation); }
        _rotationFlag = false;
        _house.localScale = _dimensions * _scale;
        if (_house.lossyScale != Vector3.zero)
        {
            scalingIsZero = false;
        }
        else
    ...
    {
        return Matrix4x4.Rotate(_house.transform.rotation);
    }
    ...
    {
        return Matrix4x4.TRS(getTranslation(), _house.transform.rotation, Vector3.one * _scale);
    }
    ...
    {
#if UNITY_EDITOR
        string path = "Assets/Resources/" + saveFolder + "/LA_House.prefab";
    ...
    public GameObject getGameObject() { return _house.gameObject; }

...
//LA_EllipticTriangularPrism
public class LA_EllipticTriangularPrism : LA_Object, SaveAssets
{
    string _name = "";
    ...
    Transform _parent;
    Transform _ellipticTriangularPrism;
    ...
    public LA_EllipticTriangularPrism(Vector3 centerPoint, Vector3 dimensions, Vector3 orientation, Color color)
    {
        ...
        GameObject temp = (GameObject)AssetDatabase.LoadAssetAtPath("Assets/Resources/Geometry/Prefab/TriangularPrism.prefab", typeof(GameObject));
        _ellipticTriangularPrism = ((GameObject)PrefabUtility.InstantiatePrefab(temp)).transform;
#else
        _ellipticTriangularPrism = GameObject.Instantiate<GameObject>(Resources.Load<GameObject>("Geometry/Prefab/TriangularPrism")).transform;
#endif
        _centerPoint = centerPoint;
        _orientation = orientation;
        _rotationFlag = true;
        _ellipticTriangularPrism.SetParent(_parent);
        ColorScripts.getRightColorScript(color, _ellipticTriangularPrism.gameObject);
        ...
        _ellipticTriangularPrism.gameObject.layer = LayerMask.NameToLayer("LA_Object");

        _toStringState = ToStringStates.oneMatrix;
    }

    public LA_EllipticTriangularPrism(Transform parent, Transform cuboid, Vector3 orientation, Vector3 centerPoint, float scale, Vector3 dimensions)
    {
        _parent = parent;
        _ellipticTriangularPrism = cuboid;
        _orientation = orientation;
        _centerPoint = centerPoint;
        _scale = scale;
    ...
    public void draw()
    {
        _ellipticTriangularPrism.position = _centerPoint;
        if (_rotationFlag) { _ellipticTriangularPrism.rotation = _ellipticTriangularPrism.rotation * Quaternion.Euler(_orientation); }
        _rotationFlag = false;
        _ellipticTriangularPrism.localScale = _dimensions * _scale;
        if (_ellipticTriangularPrism.lossyScale != Vector3.zero)
        {
            scalingIsZero = false;
        }
        else
    ...
    {
        return Matrix4x4.Rotate(_ellipticTriangularPrism.transform.rotation);
    }
    ...
    {
        return _ellipticTriangularPrism.transform.position;
    }

    public Matrix4x4 getMatrix()
    {
        return Matrix4x4.TRS(getTranslation(), _ellipticTriangularPrism.transform.rotation, Vector3.one * _scale);
    }
    ...
    {
#if UNITY_EDITOR
        string path = "Assets/Resources/" + saveFolder + "/LA_EllipticTriangularPrism.prefab";
    ...
    public GameObject getGameObject() { return _ellipticTriangularPrism.gameObject; }

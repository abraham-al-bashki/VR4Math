...
public class LA_Capsule : LA_Object, SaveAssets
{
    string _name = "";
    ...
    public LA_Capsule(Vector3 centerPoint, Vector3 dimensions, Vector3 orientation, Color color)
    {
        _parent = new GameObject().transform;
        _parent.gameObject.AddComponent<LA_Object_Component>().laobject = this;


        _capsule = GameObject.CreatePrimitive(PrimitiveType.Capsule).transform;
        _centerPoint = centerPoint;
        _orientation = orientation;
        _rotationFlag = true;
        _capsule.SetParent(_parent);
        ColorScripts.getRightColorScript(color, _capsule.gameObject);
        ...
        _capsule.gameObject.layer = LayerMask.NameToLayer("LA_Object");

        _toStringState = ToStringStates.oneMatrix;
    }

    public LA_Capsule(Transform parent, Transform capsule, Vector3 orientation, Vector3 centerPoint, float scale, Vector3 dimensions)
    {
        _parent = parent;
        _capsule = capsule;
        _orientation = orientation;
    ...
    public void draw()
    {
        _capsule.position = _centerPoint;
        if (_rotationFlag) { _capsule.rotation = _capsule.rotation * Quaternion.Euler(_orientation); }
        _rotationFlag = false;
        _capsule.localScale = _dimensions * _scale;
        if (_capsule.lossyScale != Vector3.zero)
        {
            scalingIsZero = false;
        }
        else
    ...
    {
        return Matrix4x4.Rotate(_capsule.transform.rotation);
    }
    ...
    {
        return _capsule.transform.position;
    }

    public Matrix4x4 getMatrix()
    {
        return Matrix4x4.TRS(getTranslation(), _capsule.transform.rotation, Vector3.one * _scale);
    }
    ...
    {
#if UNITY_EDITOR
        string path = "Assets/Resources/" + saveFolder + "/LA_Capsule.prefab";
    ...
    public GameObject getGameObject() { return _capsule.gameObject; }

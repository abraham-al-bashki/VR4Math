...
//LA_EllipticCone
public class LA_EllipticCone : LA_Object, SaveAssets
{
    string _name = "";
    ...
    public LA_EllipticCone(Vector3 centerPoint, Vector3 dimensions, Vector3 orientation, Color color)
    {
        ...
        GameObject temp = (GameObject)AssetDatabase.LoadAssetAtPath("Assets/Resources/Geometry/Prefab/Cone.prefab", typeof(GameObject));
        ellipticCone = ((GameObject)PrefabUtility.InstantiatePrefab(temp)).transform;
#else
        ellipticCone = GameObject.Instantiate<GameObject>(Resources.Load<GameObject>("Geometry/Prefab/Cone")).transform;
#endif
        _centerPoint = centerPoint;
        _orientation = orientation;
        _rotationFlag = true;
        ellipticCone.SetParent(_parent);
        ColorScripts.getRightColorScript(color, ellipticCone.gameObject);
        ...
        ellipticCone.gameObject.layer = LayerMask.NameToLayer("LA_Object");

        _toStringState = ToStringStates.oneMatrix;
    }

    public LA_EllipticCone(Transform parent, Transform cuboid, Vector3 orientation, Vector3 centerPoint, float scale, Vector3 dimensions)
    {
        _parent = parent;
        ellipticCone = cuboid;
        _orientation = orientation;
    ...
    public void draw()
    {
        ellipticCone.position = _centerPoint;
        if (_rotationFlag) { ellipticCone.rotation = ellipticCone.rotation * Quaternion.Euler(_orientation); }
        _rotationFlag = false;
        ellipticCone.localScale = _dimensions * _scale;
        if (ellipticCone.lossyScale != Vector3.zero)
        {
            scalingIsZero = false;
        }
        else
    ...
    {
        return Matrix4x4.Rotate(ellipticCone.transform.rotation);
    }
    ...
    {
        return ellipticCone.transform.position;
    }

    public Matrix4x4 getMatrix()
    {
        return Matrix4x4.TRS(getTranslation(), ellipticCone.transform.rotation, Vector3.one * _scale);
    }
    ...
    {
#if UNITY_EDITOR
        string path = "Assets/Resources/" + saveFolder + "/LA_EllipticCone.prefab";
    ...
    public GameObject getGameObject() { return ellipticCone.gameObject; }

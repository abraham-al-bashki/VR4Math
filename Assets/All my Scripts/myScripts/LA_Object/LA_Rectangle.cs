...
public class LA_Rectangle : LA_Object, SaveAssets
{
    string _name = "";
    ...
    public LA_Rectangle(Vector3 centerPoint, Vector3 dimensions, Vector3 orientation, Color color)
    {
        _parent = new GameObject().transform;
        _parent.gameObject.AddComponent<LA_Object_Component>().laobject = this;


        _rectangle = GameObject.CreatePrimitive(PrimitiveType.Quad).transform;
        _centerPoint = centerPoint;
        _orientation = orientation;
        _rotationFlag = true;
        _rectangle.SetParent(_parent);
        ColorScripts.getRightColorScript(color, _rectangle.gameObject);
        ...
        _rectangle.gameObject.layer = LayerMask.NameToLayer("LA_Object");

        _toStringState = ToStringStates.oneMatrix;
    }

    public LA_Rectangle(Transform parent, Transform rectangle, Vector3 orientation, Vector3 centerPoint, float scale, Vector3 dimensions)
    {
        _parent = parent;
    ...
    public void draw()
    {
        _rectangle.position = _centerPoint;
        if (_rotationFlag) { _rectangle.rotation = _rectangle.rotation * Quaternion.Euler(_orientation); }
        _rotationFlag = false;
        _rectangle.localScale = _dimensions * _scale;
        if (_rectangle.lossyScale != Vector3.zero)
        {
            scalingIsZero = false;
        }
        else
    ...
    {
        return Matrix4x4.Rotate(_rectangle.transform.rotation);
    }
    ...
    {
        return _rectangle.transform.position;
    }

    public Matrix4x4 getMatrix()
    {
        return Matrix4x4.TRS(getTranslation(), _rectangle.transform.rotation, Vector3.one * _scale);
    }
    ...
    {
#if UNITY_EDITOR
        string path = "Assets/Resources/" + saveFolder + "/LA_Rectangle.prefab";
    ...
    public GameObject getGameObject() { return _rectangle.gameObject; }

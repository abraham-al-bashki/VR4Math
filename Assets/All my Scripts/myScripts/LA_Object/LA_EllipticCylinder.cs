...
public class LA_EllipticCylinder : LA_Object, SaveAssets
{
    string _name = "";
    ...
    public LA_EllipticCylinder(Vector3 centerPoint, Vector3 dimensions, Vector3 orientation, Color color)
    {
        _parent = new GameObject().transform;
        _parent.gameObject.AddComponent<LA_Object_Component>().laobject = this;


        _ellipticCylinder = GameObject.CreatePrimitive(PrimitiveType.Cylinder).transform;
        _centerPoint = centerPoint;
        _orientation = orientation;
        _rotationFlag = true;
        _ellipticCylinder.SetParent(_parent);
        ColorScripts.getRightColorScript(color, _ellipticCylinder.gameObject);
        ...
        _ellipticCylinder.gameObject.layer = LayerMask.NameToLayer("LA_Object");

        _toStringState = ToStringStates.oneMatrix;
    }

    public LA_EllipticCylinder(Transform parent, Transform cuboid, Vector3 orientation, Vector3 centerPoint, float scale, Vector3 dimensions)
    {
        _parent = parent;
        _ellipticCylinder = cuboid;
        _orientation = orientation;
        _centerPoint = centerPoint;
        _scale = scale;
    ...
    public void draw()
    {
        _ellipticCylinder.position = _centerPoint;
        if (_rotationFlag) { _ellipticCylinder.rotation = _ellipticCylinder.rotation * Quaternion.Euler(_orientation); }
        _rotationFlag = false;
        _ellipticCylinder.localScale = _dimensions * _scale;
        if (_ellipticCylinder.lossyScale != Vector3.zero)
        {
            scalingIsZero = false;
        }
        else
    ...
    {
        return Matrix4x4.Rotate(_ellipticCylinder.transform.rotation);
    }
    ...
    {
        return _ellipticCylinder.transform.position;
    }

    public Matrix4x4 getMatrix()
    {
        return Matrix4x4.TRS(getTranslation(), _ellipticCylinder.transform.rotation, Vector3.one * _scale);
    }
    ...
    {
#if UNITY_EDITOR
        string path = "Assets/Resources/" + saveFolder + "/LA_EllipticCylinder.prefab";
    ...
    public GameObject getGameObject() { return _ellipticCylinder.gameObject; }

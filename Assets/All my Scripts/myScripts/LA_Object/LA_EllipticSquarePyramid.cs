...
public class LA_EllipticSquarePyramid : LA_Object, SaveAssets
{
    string _name = "";
    ...
    Transform _parent;
    Transform _ellipticSquarePyramid;
    ...
    public LA_EllipticSquarePyramid(Vector3 centerPoint, Vector3 dimensions, Vector3 orientation, Color color)
    {
        ...
        GameObject temp = (GameObject)AssetDatabase.LoadAssetAtPath("Assets/Resources/Geometry/Prefab/SquarePyramid.prefab", typeof(GameObject));
        _ellipticSquarePyramid = ((GameObject)PrefabUtility.InstantiatePrefab(temp)).transform;
#else
        _ellipticSquarePyramid = GameObject.Instantiate<GameObject>(Resources.Load<GameObject>("Geometry/Prefab/SquarePyramid")).transform;
#endif
        _centerPoint = centerPoint;
        _orientation = orientation;
        _rotationFlag = true;
        _ellipticSquarePyramid.SetParent(_parent);
        ColorScripts.getRightColorScript(color, _ellipticSquarePyramid.gameObject);
        ...
        _ellipticSquarePyramid.gameObject.layer = LayerMask.NameToLayer("LA_Object");

        _toStringState = ToStringStates.oneMatrix;
    }

    public LA_EllipticSquarePyramid(Transform parent, Transform ellipticSquarePyramid, Vector3 orientation, Vector3 centerPoint, float scale, Vector3 dimensions)
    {
        _parent = parent;
        _ellipticSquarePyramid = ellipticSquarePyramid;
        _orientation = orientation;
        _centerPoint = centerPoint;
        _scale = scale;
    ...
    public void draw()
    {
        _ellipticSquarePyramid.position = _centerPoint;
        if (_rotationFlag) { _ellipticSquarePyramid.rotation = _ellipticSquarePyramid.rotation * Quaternion.Euler(_orientation); }
        _rotationFlag = false;
        _ellipticSquarePyramid.localScale = _dimensions * _scale;
        if (_ellipticSquarePyramid.lossyScale != Vector3.zero)
        {
            scalingIsZero = false;
        }
        else
    ...
    {
        return Matrix4x4.Rotate(_ellipticSquarePyramid.transform.rotation);
    }
    ...
    {
        return _ellipticSquarePyramid.transform.position;
    }

    public Matrix4x4 getMatrix()
    {
        return Matrix4x4.TRS(getTranslation(), _ellipticSquarePyramid.transform.rotation, Vector3.one * _scale);
    }
    ...
    {
#if UNITY_EDITOR
        string path = "Assets/Resources/" + saveFolder + "/LA_Cuboid.prefab";
    ...
    public GameObject getGameObject() { return _ellipticSquarePyramid.gameObject; }
